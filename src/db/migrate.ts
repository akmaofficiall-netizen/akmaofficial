...
export async function migrateDatabase() {
  ...
  await db.execute(sql`
    ...
    INSERT INTO permissions(code, name) VALUES
      ...
      ('production.view','مشاهده تولید'),('inventory.view','مشاهده انبار'),('purchases.view','مشاهده خرید'),
      ('financial.view','مشاهده مالی'),('alerts.view','مشاهده اعلان‌ها'),('ai.view','مشاهده هوش مصنوعی'),('backup.view','مشاهده پشتیبان‌گیری'),('settings.view','مشاهده تنظیمات'),
      ('invoices.pay','ثبت وصول فاکتور')
    ON CONFLICT (code) DO NOTHING;
    ...
    ON CONFLICT DO NOTHING;
    INSERT INTO role_permissions(role_id, permission_id)
      SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('customers.view','customers.create','customers.update','orders.view','orders.create','orders.update','orders.cancel','invoices.view','invoices.create','invoices.update','commissions.view','reports.view','cost.view','profit.view','projects.view','projects.create','projects.update','projects.price.manage','projects.commission.manage','projects.expense.manage','customers.transfer','payments.create','employees.view','employees.manage') WHERE r.code='manager'
    ON CONFLICT DO NOTHING;
    INSERT INTO role_permissions(role_id, permission_id)
      SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('customers.view','customers.create','customers.update','invoices.view','invoices.create','invoices.update','customers.transfer','payments.create','projects.view','reports.view','products.view') WHERE r.code='visitor'
    ON CONFLICT DO NOTHING;

    INSERT INTO role_permissions(role_id, permission_id)
      SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('customers.view','invoices.view','invoices.create','invoices.update','payments.view','payments.create','reports.view','cost.view','profit.view','projects.view','products.view','raw_materials.view','suppliers.view','financial.view') WHERE r.code='accountant'
    ON CONFLICT DO NOTHING;
