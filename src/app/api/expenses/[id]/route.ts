...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await db.transaction(async (tx) => {
      ...
      if (newAmount <= 0) {
        throw new Error("مبلغ هزینه باید بزرگتر از صفر باشد.");
      }
      ...
      if (oldAccountId === newAccountId && newAccountId) {
        ...
        if (diff > 0) {
          ...
          if (currentBalance < diff) {
            throw new Error(
              `موجودی حساب «${acc.name}» کافی نیست و نمی‌تواند منفی باشد. موجودی فعلی: ${currentBalance.toLocaleString("fa-IR")} تومان، افزایش مورد نیاز: ${diff.toLocaleString("fa-IR")} تومان.`
            );
          }
        }
        ...
        if (newAccountId) {
          ...
          if (newBalance < newAmount) {
            throw new Error(
              `موجودی حساب جدید «${newAcc.name}» کافی نیست و نمی‌تواند منفی باشد. موجودی فعلی: ${newBalance.toLocaleString("fa-IR")} تومان، مبلغ هزینه: ${newAmount.toLocaleString("fa-IR")} تومان.`
            );
          }
          await tx
            .update(accounts)
    ...
    return NextResponse.json({ success: false, error: error.message }, { status: error.message?.includes("موجودی") ? 400 : 500 });
  }
}
