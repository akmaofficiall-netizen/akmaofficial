...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.paymentStatus !== undefined) {
      const next = String(body.paymentStatus);
      if (!["unpaid", "partial", "paid"].includes(next)) return NextResponse.json({ success: false, error: "وضعیت پرداخت نامعتبر است" }, { status: 400 });
      if (next === "paid" && Number(existing.balanceDue) > 0) return NextResponse.json({ success: false, error: "برای پرداخت کامل باید سند پرداخت ثبت شود." }, { status: 400 });
      if (next === "unpaid" && Number(existing.paidAmount) > 0) return NextResponse.json({ success: false, error: "این فاکتور پرداخت ثبت‌شده دارد؛ برای برگشت پرداخت باید تراکنش اصلاح شود." }, { status: 400 });
      patch.paymentStatus = next;
    }
    const [updated] = await db.update(invoices).set(patch).where(eq(invoices.id, id)).returning();
    await logAuditEvent("UPDATE", "invoice", id, { fields: Object.keys(patch) });
    return NextResponse.json({ success: true, invoice: updated });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
