...
import { db } from "@/db";
import { purchases, purchaseItems, suppliers, rawMaterials, accounts } from "@/db/schema";
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.items && Array.isArray(body.items) && body.items.length > 0) {
      ...
      for (const old of oldItems) {
        if (old.itemType === "raw_material") {
          await recordInventoryTransaction({
            ...
            quantityChange: -Number(old.quantity), // Deduct old added stock
      ...
      // 3. Insert new items and record positive stock
      grandTotal = 0;
      for (const item of body.items) {
        ...
        if (item.itemType === "raw_material" || !item.itemType) {
          await recordInventoryTransaction({
            ...
            quantityChange: qty, // Add new stock
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // 1. Reverse stock changes for all purchased items
    for (const item of items) {
      if (item.itemType === "raw_material") {
        await recordInventoryTransaction({
          ...
          itemId: item.itemId,
          transactionType: "adjustment",
          quantityChange: -Number(item.quantity), // Deduct previously added items
    ...
    // 2. Delete purchase items and purchase record
    await db.delete(purchaseItems).where(eq(purchaseItems.purchaseId, id));
