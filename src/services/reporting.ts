 ...
 */
export async function getProjectComparisonReport(projectAId: string, projectBId: string) {
  const reportA = await getDashboardKPIs({ projectId: projectAId });
  const reportB = await getDashboardKPIs({ projectId: projectBId });
