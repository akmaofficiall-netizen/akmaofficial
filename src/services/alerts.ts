...
export async function runAlertsEngineScan() {
    ...
    if (!existing) {
      const [newAlert] = await db
        ...
        .values({
          ...
          message: `موجودی ماده اولیه "${rm.name}" برابر ${rm.stockQuantity} ${rm.unit} است که کمتر از حداقل حد مجاز (${rm.minStockQuantity} ${rm.unit}) می‌باشد.`,
    ...
    if (!existing) {
      const [newAlert] = await db
        ...
        .values({
          ...
          message: `موجودی محصول "${p.name}" (${p.stockQuantity} ${p.unit}) کمتر از حداقل سفارش (${p.minStockQuantity} ${p.unit}) است.`,
          entityType: "product",
          entityId: p.id,
          dedupKey,
        })
  ...
  for (const inv of overdueInvoices) {
    ...
    const [existing] = await db.select({ id: alerts.id }).from(alerts).where(eq(alerts.dedupKey, dedupKey)).limit(1);
    if (!existing) {
      const balanceNum = Number(inv.balanceDue).toLocaleString("fa-IR");
      const [newAlert] = await db
        .insert(alerts)
        .values({
