...
const secret=()=>{ const value=process.env.AUTH_SECRET||process.env.AUTH_SALT; if(value) return value; if(process.env.NODE_ENV==="production") throw new Error("AUTH_SECRET در محیط Production تنظیم نشده است."); return "AKMA_DEV_AUTH_SECRET"; };
...
export async function ensureDefaultAdminAccount() {
  const username = process.env.INITIAL_ADMIN_USERNAME || "akmaadmin";
  const password = process.env.INITIAL_ADMIN_PASSWORD || "AkmaAdmin@2026";
  const [existingByUsername] = await db.select({ id: employeeAccounts.id }).from(employeeAccounts).where(eq(employeeAccounts.username, username)).limit(1);
  if (existingByUsername) return;
  let [employee] = await db.select().from(employees).where(eq(employees.code, "EMP-ADMIN-001")).limit(1);
  if (employee) { const [existingByEmployee] = await db.select({ id: employeeAccounts.id }).from(employeeAccounts).where(eq(employeeAccounts.employeeId, employee.id)).limit(1); if (existingByEmployee) return; }
  if (!employee) {
