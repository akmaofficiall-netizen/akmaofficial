...
export const InvoicesView: React.FC<{ selectedProjectId: string | null }> = ({ selectedProjectId }) => {
  ...
  const [paymentForm, setPaymentForm] = useState({ amount:0, accountId:"", paymentMethod:"pos", referenceNumber:"", notes:"" });
  ...
  const openEditInvoice = async (inv:any) => { const res=await fetch(`/api/invoices/${inv.id}`).then(r=>r.json()); if(!res.success) return alert(res.error||"خطا"); setEditingInvoice({ ...res.invoice, dueDate:res.invoice.dueDate?String(res.invoice.dueDate).slice(0,10):"" }); setInvoicePayments(res.payments||[]); setPaymentForm({amount:0,accountId:accounts[0]?.id||"",paymentMethod:"pos",referenceNumber:"",notes:""}); };
  const saveInvoiceEdit = async (e:React.FormEvent) => { e.preventDefault(); if(!editingInvoice) return; setSaving(true); try { const res=await fetch(`/api/invoices/${editingInvoice.id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({invoiceDate:editingInvoice.invoiceDate,dueDate:editingInvoice.dueDate,notes:editingInvoice.notes})}).then(r=>r.json()); if(!res.success) throw new Error(res.error||"خطا"); setEditingInvoice(null); fetchData(); } catch(e:any){ alert(e.message);} finally{setSaving(false);} };
  const addInvoicePayment = async () => { if(!editingInvoice || !paymentForm.accountId || paymentForm.amount<=0) return alert("مبلغ و حساب را انتخاب کنید"); setSaving(true); try { const res=await fetch("/api/payments",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({invoiceId:editingInvoice.id,customerId:editingInvoice.customerId,projectId:editingInvoice.projectId,accountId:paymentForm.accountId,amount:paymentForm.amount,paymentMethod:paymentForm.paymentMethod,referenceNumber:paymentForm.referenceNumber,notes:paymentForm.notes})}).then(r=>r.json()); if(!res.success) throw new Error(res.error||"خطا در ثبت پرداخت"); const next=await fetch(`/api/invoices/${editingInvoice.id}`).then(r=>r.json()); if(next.success){setEditingInvoice(next.invoice);setInvoicePayments(next.payments||[]);} setPaymentForm({amount:0,accountId:accounts[0]?.id||"",paymentMethod:"pos",referenceNumber:"",notes:""}); fetchData(); } catch(e:any){ alert(e.message);} finally{setSaving(false);} };
  ...
  return (
    <div className="space-y-6">
                ...
                <tr key={inv.id} className="hover:bg-slate-800/40 transition-all">
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {inv.status !== "reversed" && <button onClick={()=>openEditInvoice(inv)} title="ویرایش فاکتور و ثبت تسویه" className="rounded-lg bg-cyan-500/10 p-1.5 text-cyan-300 hover:bg-cyan-500/20"><FileText className="h-3.5 w-3.5"/></button>}
                      {inv.status !== "reversed" && (
                        <button
                          onClick={() => {
      ...
      {/* Reversal Modal */}
      {editingInvoice && <div className="fixed inset-0 z-[65] bg-black/80 flex items-center justify-center p-4 overflow-y-auto"><div className="w-full max-w-3xl rounded-2xl border border-slate-800 bg-slate-950 p-6 space-y-5 my-6"><div className="flex justify-between"><h3 className="font-bold">ویرایش فاکتور {editingInvoice.invoiceNumber}</h3><button onClick={()=>setEditingInvoice(null)}><X className="h-5 w-5"/></button></div><div className="grid md:grid-cols-4 gap-3"><div className="rounded-xl bg-slate-900 p-3"><span className="text-[10px] text-slate-500">کل فاکتور</span><b className="block mt-1 text-emerald-300">{Number(editingInvoice.grandTotal).toLocaleString("fa-IR")}</b></div><div className="rounded-xl bg-slate-900 p-3"><span className="text-[10px] text-slate-500">تسویه شده</span><b className="block mt-1 text-cyan-300">{Number(editingInvoice.paidAmount).toLocaleString("fa-IR")}</b></div><div className="rounded-xl bg-slate-900 p-3"><span className="text-[10px] text-slate-500">مانده</span><b className="block mt-1 text-rose-300">{Number(editingInvoice.balanceDue).toLocaleString("fa-IR")}</b></div><div className="rounded-xl bg-slate-900 p-3"><span className="text-[10px] text-slate-500">وضعیت</span><b className="block mt-1">{editingInvoice.paymentStatus==='paid'?"کامل تسویه شده":editingInvoice.paymentStatus==='partial'?"تسویه جزئی":"تسویه نشده"}</b></div></div><form onSubmit={saveInvoiceEdit} className="space-y-3"><input type="date" value={editingInvoice.invoiceDate?String(editingInvoice.invoiceDate).slice(0,10):""} onChange={e=>setEditingInvoice({...editingInvoice,invoiceDate:e.target.value})} className="rounded-xl border border-slate-800 bg-slate-900 p-3 text-white"/><input type="date" value={editingInvoice.dueDate||""} onChange={e=>setEditingInvoice({...editingInvoice,dueDate:e.target.value})} className="rounded-xl border border-slate-800 bg-slate-900 p-3 text-white"/><textarea value={editingInvoice.notes||""} onChange={e=>setEditingInvoice({...editingInvoice,notes:e.target.value})} className="w-full rounded-xl border border-slate-800 bg-slate-900 p-3 text-white" placeholder="توضیحات"/><button disabled={saving} className="rounded-xl bg-cyan-600 px-4 py-2 font-bold">ذخیره اطلاعات فاکتور</button></form><div className="border-t border-slate-800 pt-4 space-y-3"><h4 className="font-bold">ثبت تسویه جدید</h4><div className="grid md:grid-cols-4 gap-2"><input type="number" value={paymentForm.amount||""} onChange={e=>setPaymentForm({...paymentForm,amount:Number(e.target.value)})} placeholder="مبلغ" className="rounded-xl bg-slate-900 border border-slate-800 p-3"/><select value={paymentForm.accountId} onChange={e=>setPaymentForm({...paymentForm,accountId:e.target.value})} className="rounded-xl bg-slate-900 border border-slate-800 p-3"><option value="">حساب</option>{accounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><select value={paymentForm.paymentMethod} onChange={e=>setPaymentForm({...paymentForm,paymentMethod:e.target.value})} className="rounded-xl bg-slate-900 border border-slate-800 p-3"><option value="pos">POS</option><option value="cash">نقدی</option><option value="card_transfer">کارت به کارت</option><option value="bank_transfer">انتقال بانکی</option><option value="cheque">چک</option></select><button type="button" onClick={addInvoicePayment} className="rounded-xl bg-emerald-600 px-4 py-2 font-bold">ثبت پرداخت</button></div></div><div className="space-y-2"><h4 className="font-bold">تاریخچه پرداخت‌ها</h4>{invoicePayments.map((p:any)=><div key={p.id} className="flex justify-between rounded-xl bg-slate-900 p-3 text-xs"><span>{p.paymentNumber} · {p.paymentMethod}</span><b>{Number(p.amount).toLocaleString("fa-IR")}</b></div>)}{!invoicePayments.length&&<p className="text-xs text-slate-500">هنوز پرداختی ثبت نشده است.</p>}</div></div></div>}
