        ...
        {filtered.map((c) => (
          ...
          >
            ...
            <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
              ...
              <button onClick={() => { setEditingCustomer(c); setFormData({ code:c.code, name:c.name, storeName:c.storeName||"", mobile:c.mobile, phone:c.phone||"", email:c.email||"", address:c.address||"", city:c.city||"تهران", latitude:Number(c.latitude)||35.6892, longitude:Number(c.longitude)||51.389, assignedEmployeeId:c.assignedEmployeeId||"", notes:c.notes||"" }); }} className="rounded-lg border border-slate-700 px-2 py-1 text-[11px] text-cyan-300">ویرایش</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSaveCustomer} className="space-y-4 text-xs">
              ...
              <GoogleMapPicker latitude={formData.latitude} longitude={formData.longitude} onChange={v=>setFormData({...formData,...v})}/>
      ...
      {editingCustomer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 overflow-y-auto">
          <div className="w-full max-w-3xl rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4"><h3 className="font-bold text-white">ویرایش مشتری: {editingCustomer.name}</h3><button onClick={()=>setEditingCustomer(null)}><X className="h-5 w-5 text-slate-400"/></button></div>
            <div className="grid md:grid-cols-2 gap-3">
              {[["name","نام"],["storeName","فروشگاه"],["mobile","موبایل"],["phone","تلفن"],["city","شهر"],["address","آدرس"]].map(([k,l])=><label key={k} className="text-xs text-slate-400">{l}<input value={(formData as any)[k]||""} onChange={e=>setFormData({...formData,[k]:e.target.value})} className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 p-2.5 text-white"/></label>)}
            </div>
            <div className="mt-4"><GoogleMapPicker latitude={formData.latitude} longitude={formData.longitude} onChange={v=>setFormData({...formData,...v})}/></div>
            <div className="flex justify-end gap-2 mt-4"><button onClick={()=>setEditingCustomer(null)} className="rounded-xl border border-slate-700 px-4 py-2 text-xs">انصراف</button><button onClick={async()=>{const r=await fetch(`/api/customers/${editingCustomer.id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(formData)}).then(x=>x.json()); if(r.success){setEditingCustomer(null);fetchData();} else alert(r.error||"خطا");}} className="rounded-xl bg-cyan-600 px-4 py-2 text-xs font-bold">ذخیره</button></div>
          </div>
        </div>
      )}
